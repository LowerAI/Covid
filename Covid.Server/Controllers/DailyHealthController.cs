...
namespace Covid.Server.Controllers
{
    ...
    public class DailyHealthController : ControllerBase
    {
        ...
        public async Task<ActionResult<IList<DailyHealthDto>>> Get(int departmentId, DateTime date)
        {
            var entities = await _dailyHealthRepository.GetByDepartmentAsync(departmentId, date);
        ...
        public async Task<ActionResult<IList<DailyHealthDto>>> Get(int departmentId, DateTime date, List<DailyHealthDto> dailyHealths)
        {
            ...
            var updateEntities = await _dailyHealthRepository.GetByDepartmentAsync(departmentId, date);
            var dtos = updateEntities.Select(x => new DailyHealthDto
            {
                Date = x.Date,
                EmployeeId = x.EmployeeId,
                HealthCondition = x.HealthCondition,
